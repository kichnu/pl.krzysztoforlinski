...
function createArrayColors() {
    const colors = ['red', 'green', 'yellow', 'blue', 'lime', 'purple', 'olive', 'yellow', 'navy', 'blue', 'teal', 'aqua', 'orange', 'cadetblue', 'silver', 'gray', 'fuchsia', 'white', 'maroon'];
...
function randomColorBlock() {
    return parameters.arrayColors[(Math.floor(Math.random() * parameters.arrayColors.length))];
}
...
function createSpeedMoveDown(factor = 100) {
    parameters.speedMoveDown = ((25 / parameters.numberRows) / parameters.numberLevels) * factor;
}
...
function draw() {
    for (let i = 0; i < listBlocks.length; i++) {
        ...
        ctx.fillRect(listBlocks[i].x * data.widthBlock, listBlocks[i].y * data.heightBlock, data.widthBlock, data.heightBlock);
    }
}
...
function canMove() {
    ...
    for (let j = 0; j < listBlocks.length; j++) {
        if (listBlocks[index].x === listBlocks[j].x + 1 && listBlocks[index].y === listBlocks[j].y) canLeft = false;
    }
    ...
    for (let j = 0; j < listBlocks.length; j++) {
        if (listBlocks[index].x === listBlocks[j].x - 1 && listBlocks[index].y === listBlocks[j].y) canRight = false;
    }
    ...
    if (listBlocks.length === 1 && listBlocks[index].y === parameters.numberRows - 1) canDown = false;
